...
export default function VerifyEmailClient() {
  ...
  const loginHref = `/ops/login?${new URLSearchParams({
    email,
    tenant,
    welcome: "1",
  }).toString()}`;
      ...
      <CardContent className="space-y-4">
        ...
        {status === "error" ? (
          <div className="space-y-3">
            <Link
              href={`/ops/verify-email-pending?${new URLSearchParams({ email, tenant }).toString()}`}
            ...
            <p className="text-center text-xs text-muted">
              <Link href="/ops/login" className="text-primary hover:underline">
                Back to sign in
              </Link>
            </p>
          </div>
